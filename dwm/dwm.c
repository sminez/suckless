...
#define ISVISIBLE(C, M)         ((C->tags & M->tagset[M->seltags]) || C->ispinned)
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, ispinned;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
static void togglepinned(const Arg *arg);
...
void
runAutostart(void) {
	system("cd ~/bin/scripts; ./start-dwm.sh &");
}
...
void
togglepinned(const Arg *arg)
{
	if (!selmon->sel)
		return;
	selmon->sel->ispinned = !selmon->sel->ispinned;
	arrange(selmon);
}
